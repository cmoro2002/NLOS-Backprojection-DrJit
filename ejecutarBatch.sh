...
do
    ...
    # Eliminar imágenes hasta que queden numImages
    totalImages=$(ls $currentFolder | wc -l)
    imagesToDelete=$((totalImages - numImages))
    
    if [ $imagesToDelete -gt 0 ]; then
        ls $currentFolder | head -n $imagesToDelete | xargs -I {} rm $currentFolder/{}
    fi
    ...
    logFile="$logsFolder/log_${numImages}.txt"
